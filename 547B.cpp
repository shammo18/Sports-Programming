...
int main(){
    ...
    int n;
    cin >> n;
    vector<int> arr(n + 5) , left(n + 1) , right(n + 1);
    set<int> st;
    ...
    for(int i = 1;i <= n;i++){
    	while(stk.size() > 0 && arr[stk.top()] >= arr[i]) stk.pop();
    	if(stk.size() > 0){
           left[i] = stk.top();
    	}
    	stk.push(i);
    ...
    //debug()  ,dbg(left) , dbg(right);

    for(int i = 1;i <= n;i++){
    	dp[comp[arr[i]]] = max(dp[comp[arr[i]]] , right[i] - left[i] - 1);
    }
    ...
    //debug() , dbg(dp[-96204862]);

    int cur = 1;
    for(int i = ptr - 1;i >= 1;i--){
       for(;cur <= min(dp[i] , n);cur++){
       	 cout << decomp[i] <<  " ";
       }
       if(cur > n) break;
    }
