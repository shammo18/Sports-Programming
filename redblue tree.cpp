...
vector<int> g[mxn];    // red = +1 , black = -1
int bal[mxn] , depth[mxn];
int ans = 0;
...
void solve() {
    ...
    dfs(1 , 0 , 0);
    for (int i = 2; i <= n; i++) ans += abs(bal[i]);
    bool flag;
    if (bal[1] != 0)ans  = 1e9;
    for(int i = 0;i <= n;i++) bal[i] = 0;
    dfs2(1 , 0);
    int ans2 = 0;
    for (int i = 2; i <= n; i++) ans2 += abs(bal[i]);
    if (bal[1] != 0) ans2 = 1e9;
