...
int solve(int pos , double temp, double tot){
      
      if(tot >= P) return -100000000;
      if(pos >= n){
      	if(tot < P) return 0;
      	else return -100000000;
      }

	  int ret1 = solve(pos + 1 , temp * (1.0 - p[pos]) , tot +  p[pos]) + m[pos];
	  int ret2 = solve(pos + 1 , temp , tot);
      
      return max(ret1 , ret2);
}  

int solve(int pos , int tk){
