...
void build(int node, int b , int e) {
	...
	seg[node].val = min(seg[left].val , seg[right].val);
}
...
void update(int node , int b , int e , int l , int r , ll x) {
	...
	if (seg[node].lazy != 0) {
		...
		seg[left].val += seg[node].lazy;
		seg[right].val += seg[node].lazy;
		seg[node].lazy = 0;
...
void solve() {
	...
	for (int i = 1; i <= n; i++) {
		...
		if (pos + 1 <= n)update(1 , 1 , n , pos + 1, n , -i);
