...
int can(string s,int i, int k){
    bool flag;
        ...
        for(int j = 0;j < 7;j++){
        	...
        	if(s[j] == '0' && v[i][j] == '1')cnt++;
        }

        if(flag && cnt <= k) return  k - cnt;
...
bool solve(int pos , int k){
      ...
      bool ret = false;
      for(int i = 9;i >= 0;i--){
      	  int temp = can(arr[pos] , i , k);

      	  if(temp == -1) continue;
          ans.pb(i);
      	  ret |= solve(pos + 1 , temp);
          ans.pop_back();
      }
...
*/
   ...
   mem(dp , -1);
   v = {"1110111", "0010010", "1011101", "1011011", "0111010", "1101011", "1101111", "1010010", "1111111", "1111011"};
   cin >> n >> k;
