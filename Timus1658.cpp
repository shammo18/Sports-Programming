...
bool solve(int pos,int d){
       ...
       bool ret = 0;
       for(int i = 1;i <= 9;i++)
	       if(d + i * i <= s2)ret = ret | solve(pos + 1,d + i * i);

	    return dp[pos][d] =  ret;
}

bool solve2(int pos,int d){

       if(d == s2) return true;
       if(pos > 100) return 0;
       if(dp2[pos][d] != -1) return dp2[pos][d];  
       bool ret = 0;
       for(int i = 1;i <= 9;i++)
	       if(d + i <= s1)ret = ret | solve2(pos + 1,d + i);

	    return dp2[pos][d] =  ret;
}
