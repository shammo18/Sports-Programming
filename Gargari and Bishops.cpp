...
*/
   ...
   for(int i = 1;i <= n;i++){
   	   for(int j = 1;j <= n;j++){
   	   	   //cout << dan[i - j] << endl;
           if((i + j) & 1) odd = max(dan[i - j] + bam[i + j] - grid[i][j] , odd);
   	       else even = max(dan[i - j] + bam[i + j] - grid[i][j] , even);
   	   }
   }
   ...
   // 	   for(int j = 1;j <= n;j++){
   // 	   	   //cout << dan[i - j] << endl;
   //         if((i + j) & 1) odd = max(dan[i - j] + bam[i + j] , odd);
   // 	       else even = max(dan[i - j] + bam[i + j] , even);
   // 	       if(odd == 7) {
   // 	       	cout << i <<" " << j << endl;
   // 	       	return 0;
   // 	       }
   ...
   for(int i = 1;i <= n;i++){
   	   for(int j = 1;j <= n;j++){
           if((i + j) & 1){
           	  ans = max(ans , even + dan[i - j] + bam[i + j] - grid[i][j]);
              if(ans == even + dan[i - j] + bam[i + j] - grid[i][j]){
              	 a = {i , j};
              }
           }else {
           	  ans = max(ans , odd + dan[i - j] + bam[i + j] - grid[i][j]);
           	  if(ans == odd + dan[i - j] + bam[i + j] - grid[i][j]){
           	  	b = {i , j};
           	  }
           }
   	   }
