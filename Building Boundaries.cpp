  	...
  	for(int i = 0;i <= 7;i++){
  		...
  		do{
           ...
           // 	cout << endl;
  		   ...
  		   pll one = temp[idx[0]] , two = temp[idx[1]] , three = temp[idx[2]];
  		   ll x , y  ;
  		     x = (one.xx + two.xx + three.xx) * max({one.yy , two.yy , three.yy});

             if(two.xx <= one.xx){
             	   y = (one.xx + three.xx) * max(one.yy + two.yy , three.yy);
             }else{
             	if(three.yy <= one.yy){
             		y = max(one.xx + three.xx , two.xx) * (one.yy + two.yy);
             	}else{
             		y = (two.xx + three.xx) * max(one.yy + two.yy , three.yy);
             	}
             }
