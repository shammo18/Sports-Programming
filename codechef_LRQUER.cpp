...
void mergeSort(int node, int l, int h)
{
    ...
    adj[node].clear();
	adj[node].insert(all(adj[node * 2]));
	adj[node].insert(all(adj[node * 2 + 1]));
...
ll query(int node,int b,int e,int l,int r,ll x){
        ...
        if(b > r || e < l) return 1e15;
        if(b >= l && e <= r){
           auto pos1 = adj[node].lower_bound(x);
           auto pos2 = adj[node].upper_bound(x);
           
           if(pos2 != adj[node].begin()) pos2--;
           if(pos1 == adj[node].end())pos1--;

           if(abs(*pos1 - x) <= abs(*pos2 - x)) return *pos1;
           else return *pos2;
        ...
        int mid = b + e >> 1;
        ll xx = query(node << 1,b,mid,l,r,x);
        ll y = query(node << 1 | 1, mid + 1 , e,l,r,x);
		
           if(abs(xx - x) <= abs(y - x)) return xx;
           else return y;
...
void update(int node, int b , int e , int pos ,  ll x , ll y){

        if(pos >= b && pos <= e){
           adj[node].erase(adj[node].find(y));
           adj[node].insert(x);
        if(b == e){
           return;
        }
        ...
        int mid = b + e >> 1;
        update(node << 1,b,mid,pos,x , y);
        update(node << 1 | 1, mid + 1 , e,pos , x , y);
...
*/
    ...
    while(q--){
        ...
        if(t == 1){
        	r--;
           ll m = query(1 , 0 , n - 1, l , r , (arr[l] + arr[r]) / 2);
           ll ans = (m - arr[l]) * (arr[r] - m);
           //cout << ans << endl;
           printf("%lld\n", ans);
        }else{
          ll y = arr[l];
