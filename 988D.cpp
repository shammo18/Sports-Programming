...
*/
 ...
 for(int j = 1 , cnt = 1;cnt < 32; cnt++){

 	// cout << j << endl;

  for(int i = 0;i < n;i++){
     if(arr[i] + j <= 1000000000 && arr[i] - j >= -1000000000 &&  mp[arr[i] + j] && mp[arr[i] - j]){
     	cout << 3 << endl;
     	cout << arr[i] + j << " " << arr[i] - j << " " << arr[i] << endl;
     	return 0;
     }

     if(arr[i] + j <= 1000000000 && mp[arr[i] + j]){
     	flag = true;
     	p = {arr[i] , arr[i] + j};
     }
  }
