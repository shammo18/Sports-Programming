...
ll solve(int pos,int five,int cnt){
    ...
    if(dp[pos][five][cnt] != -1) return dp[pos][five][cnt];
    ll ret = 0;
    if(cnt + 1 <= k)ret = max(ret , solve(pos + 1,five + val[pos].yy , cnt + 1) + val[pos].xx);
    ret = max(ret, solve(pos + 1,five,cnt));
    
    return dp[pos][five][cnt] = ret;
}
