...
int main(){
    ...
    for(int j = 0;j < k;j++){
    	...
    	for(int i = 0;i < n;i++){
            ...
            if(mp[sum - v[i]].size() < 2)mp[sum - v[i]].insert(j);
    ...
    for(int j = 0;j < k;j++){
    	for(int i = 0;i < adj[j].size();i++){
            int temp = s[j] - adj[j][i];
            if(mp[temp].size() <= 1)continue;
            auto itr = mp[temp].begin();
         
            if(j != *itr){
            	cout << "YES" << endl;
            	cout << j + 1 << " " << i + 1 << endl;

            	for(int l = 0;l < adj[*itr].size();l++){
            		if(s[*itr] - adj[*itr][l] == temp){
            			cout << *itr + 1 << " " << l + 1 << endl;
            			return 0;
            		}
            	}
