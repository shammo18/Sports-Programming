...
*/
	...
	for(int i = 0;i < n;i++) ses[arr[i]] = i;
	for(int i = n - 1;i >= 0;i--) suru[arr[i]] = i;
    ...
    while(1){
       ...
       // cout << l << " " << r <<" " << k << endl;
        ...
        if(bame <= dane){
        	ll temp =  k - bame * (arr[l + 1] - arr[l]);
        	if(temp <= 0){
        		ans = arr[r] - (arr[l] + k / bame);
                break;
        	}else{
        		l++;
        		k = temp;
            ...
            ll temp =  k - dane * (arr[r] - arr[r - 1]);
        	if(temp <= 0){
        		ans = (arr[r] - k / dane) - arr[l];
                break;
        	}else{
        		r--;
        		k = temp;
