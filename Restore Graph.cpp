...
*/
    ...
    if(lvl[0].size() != 1) return cout << -1 << endl ,0;
    ...
    for(int i = 1;i <= mx;i++){
        ...
        int q = t / s;
        if(t % s != 0) q++;
        if(i == 1 && q > k) return cout << -1 << endl , 0;
        else if(i != 1 && q + 1 > k) return cout << -1<< endl , 0;
   
        for(int j = 0;j < lvl[i].size();j++){
        	edges.pb({lvl[i][j] , lvl[i - 1][j % s]});
        }
    }

    cout << edges.size() << endl;
    for(auto u : edges) cout << u.xx <<" " << u.yy << endl;
