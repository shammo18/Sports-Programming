...
ll go(int pos, int is_small , int prev , int cnt){
   
   if(pos == 31){
   	 return cnt;
   }
   
   if(dp[pos][is_small][prev][cnt] != -1) return dp[pos][is_small][prev][cnt];
   int mx = is_small ? 1 : (checkbit(n , 31 - pos - 1) ? 1 : 0);
   //debug() , dbg(mx);
   ll ret = 0;
   for(int i = 0;i < mx;i++){
   	 int cost = 0;
   	 if(prev == 1 && i == 1) cost = 1; 
   	 ret += go(pos + 1, 1 , i , cnt + cost);
   }

   if(is_small){
   	 int cost = 0;
   	 if(prev == 1 && mx == 1) cost = 1; 
   	 ret += go(pos + 1, 1 , mx , cnt + cost) ;   	 
   }else{
   	 int cost = 0;
   	 if(prev == 1 && mx == 1) cost = 1; 
   	 ret += go(pos + 1, 0 , mx , cnt + cost);    	
   } 
   return dp[pos][is_small][prev][cnt] = ret;
}
