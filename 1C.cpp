...
ll solve(int pos , int prev , int cnt){
     ...
     if(prev < 0){
     	if(dp[pos][1003] != -1) return dp[pos][1003];
     }else if(dp[pos][prev] != -1) return dp[pos][prev];
     

     ll ret = 0;

     ret = (ret % mod + solve(pos + 1, prev ,cnt ) % mod ) % mod;
    
     bool flag = true;
     for(int i = 0;i <  26;i++){
     	int x = 0;
     	if(prev < 0)x = cum[i][pos];
        x = cum[i][pos] - cum[i][prev];
         if(x > 0 && pos - prev > fr[i]){
           flag = false;
           break;
         }
     }
     ...
     if(flag){
     	mx = max(mx , pos - prev); 
     	ret = (ret % mod + solve(pos + 1,pos , cnt + 1) % mod) % mod;
     }



     
     if(prev < 0)dp[pos][1003] = ret;
     else dp[pos][prev] = ret;
