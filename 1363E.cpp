...
void dfs(int u , int p , ll mn){
  ...
  for(auto v : adj[u]){
    ...
    ans += 2LL * min(pos[u] , neg[u]) * cost[u];
    ll temp = min(pos[u] , neg[u]);
    pos[u] -= temp;
    neg[u] -= temp;
...
*/
  ...
  if(z != 0 || o != 0) return cout << -1 << endl , 0;
