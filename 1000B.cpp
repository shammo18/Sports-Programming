...
ll solve(ll pos, int flag){
    ...
    ll cost = 0 , extra = 0;
	if(flag == 0){
		if(pos & 1)cost += arr[pos], extra += arr[pos] - 1;
		else cost -= arr[pos] , extra -= arr[pos - 1] + 1;


	}else{
     	if(pos & 1)cost -= arr[pos];
		else cost += arr[pos];
	}
    
    

	//if(flag == 1 && pos == n + 1 && cost < 0)return 0;

    ll ret = 0;
    if(pos != n + 1 || cost > 0)ret = max(ret, solve(pos + 1,flag) + cost);
	if(flag == 0) ret = max(ret, solve(pos,1) + extra);
