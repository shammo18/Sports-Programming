...
void solve(){
        ...
        int type , u , v , T , l , r  , ptr = 0;

	    while(q--){

	    	sci(type);
	    	if(type == 0){           // make U V friends
	    		scii(u , v);
             //   merge(u , v);
         u = find_par(u);
         v = find_par(v);
         ...
         if((int)os[u].size() >= (int)os[v].size()){
         for(auto it : os[v]){
         	os[u].insert(it);
         }
         ...
         par[v] = u;
           ...
           for(auto it : os[u]){
	    	...
	    	}else if(type == 1){     //  U post in time T
                scii(u , T);
                int p = find_par(u);
                os[p].insert({T , ++ptr});


	    	}else{                   // query L...R for U
                sciii(u , l , r);
                int p = find_par(u);
                int ans = os[p].order_of_key({r + 1 , -2e9}) - os[p].order_of_key({l  ,-2e9}); 
