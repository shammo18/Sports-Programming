...
string ff(int a, int b, int c , int d , int e ,int f, string s){
        string temp;
	    temp.pb(s[a]);
        temp.pb(s[b]);
        temp.pb(s[c]);
...
bool ok(string s, string temp){

	   if(s[0] == temp[0] && s[5] == temp[5]){

         for(int j = 0;j < 4;j++){
          bool flag = true;
          for(int i = j , k = 1;k < 5;i++ , k++){
             if(s[k] != temp[(i % 4) + 1]){
             	flag = false;
             	break;
             }
...
int main(){
    ...
    while(cin >> s){
        ...
        string temp = ff(6,7,9,10,8,11,s);
        ...
        b = ok(lol,temp);
        temp = ff(7,6,8,11,9,10,s);
        c = ok(lol,temp);
        swap(temp[0],temp[5]);
        swap(temp[2],temp[4]);

        d = ok(lol,temp);
        temp = ff(8,6,10,11,7,9,s);
        e = ok(lol,temp);
