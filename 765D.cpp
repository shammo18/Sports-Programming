...
int g[mxn] , f[mxn] , inverse_of_h[mxn];
...
*/
  ...
  for(int i = 1;i <= n;i++){
  	cin >> f[i];
  	if(f[i] == i) { h.pb(f[i]); inverse_of_h[f[i]] = (int)h.size() - 1;}
  }
  if(h.size() == 1) return cout << -1 << endl , 0;
  for(int i = 1;i <= n;i++){
  	 if(inverse_of_h[f[i]] == 0) return cout << -1 << endl , 0;
  	 g[i] = inverse_of_h[f[i]];
  }

  cout << (int)h.size() - 1 << endl;
  for(int i = 1;i <= n;i++) cout << g[i] <<" "; cout << endl;
  for(int i = 1;i < h.size();i++) cout <<h[i] <<" "; cout << endl;
