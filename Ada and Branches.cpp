...
int par[mxn] , rnk[mxn] , componentSize[mxn] , ans[mxn];
...
*/
    ...
    for(int i = 0;i < arr.size();i++){
    	//cout << arr[i].type <<" " << arr[i].w << endl;
       if(arr[i].type == 0){
          int one = find_par(arr[i].u);
          int two = find_par(arr[i].v);

          union_sets(one , two);

          if(one != two){
          	 componentSize[find_par(one)] = componentSize[one] + componentSize[two];  
          }

       }else{
          ans[arr[i].id] = componentSize[find_par(arr[i].u)];  
       }
    }
