...
bool go(int pos , int amt ,int flag){

	   if(pos >= n){
	   	  if(flag == 1 && amt == 0) return dp[pos][amt][flag] = 1;
	   	  else return dp[pos][amt][flag] = 0;
	   }

	   if(dp[pos][amt][flag] != -1) return dp[pos][amt][flag];
       
       bool ret = false;
	   if(flag == 0)ret = ret | go(pos + 1 , amt , 1);

	   if(arr[pos] <= amt) ret = ret | go(pos + 1, amt - arr[pos] , flag);
	   if(brr[pos] <= amt) ret = ret | go(pos + 1, amt - brr[pos] , flag);
       
       return dp[pos][amt][flag] = ret;
}

void sol(int pos , int amt , int flag){

	   if(pos >= n){
	   	  if(flag == 1 && amt == 0) return ;
	   	  else return;	   }
    

     if(dp[pos + 1][amt - arr[pos]][flag] == 1){
     	cout <<  1 << " ";
     	sol(pos + 1 , amt - arr[pos] , flag);

     }else if(dp[pos + 1][amt - brr[pos]][flag] == 1){
     	cout << 2 <<" ";
     	sol(pos + 1 , amt - brr[pos] , flag);
