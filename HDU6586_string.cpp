...
*/
    ...
    for(int i = 1;i <= k;i++){
       ...
       for(char ch = 'a';ch <= 'z';ch++){
       	  ...
       	  if(range[ch - 'a'].yy == 0) continue;

          auto pos = upper_bound(all(adj[ch - 'a']) , last_pos);
          
          if(pos == adj[ch - 'a'].end()) continue;

          int curr_pos = *pos;
          
          // can ch be placed into ith position ?
          ...
          pii temp = range[ch - 'a'];
          	...
          	range[ch - 'a'].xx = max( 0 , range[ch - 'a'].xx - 1);
          	range[ch - 'a'].yy = max( 0 , range[ch - 'a'].yy - 1);

          for(char ptr = 'a';ptr <= 'z';ptr++){
          	 
             if(range[ptr - 'a'].xx > cum[ptr - 'a'][n] - cum[ptr - 'a'][curr_pos]){
          	 	flag = false;
          	 	break;
          	 }

          	 cnt_mn += range[ptr - 'a'].xx;
             cnt_mx += min( range[ptr - 'a'].yy  , cum[ptr - 'a'][n] - cum[ptr - 'a'][curr_pos]);

          }

          range[ch - 'a'] = temp;

          if(cnt_mn <= baki && cnt_mx >= baki && flag){
          	ans.pb(ch);
          	last_pos = curr_pos;
          	range[ch - 'a'].xx = max( 0 , range[ch - 'a'].xx - 1);
          	range[ch - 'a'].yy = max( 0 , range[ch - 'a'].yy - 1);
