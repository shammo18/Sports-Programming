...
bool dfs(int u , int val){
	  ...
	  for(auto v : adj[u]){
         ...
         if(cost[v] == val) return false;
         flag = true;
         if(cost[v] < val && !mp[cost[v]]) {cnt++; mp[cost[v]] = true;}

	  }

	  if(flag == false){
         if(val == 1){
	     cost[u] = val; return true;
	     }
       else return false;
	}
...
*/
    ...
    for(int i = 0;i < n;i++){
        if(dfs(v[i].yy , v[i].xx) == false) return cout << -1 << endl,  0;
    }
