...
void solve(){
    ...
    for(int i = 1;i <= ptr;i++){
    	...
    	if(res != -1){
    		if(cum[res] - cum[i - 1] > best){
    			best = cum[res] - cum[i - 1];
    			taken = {i , res};
    	...
    	if(res != -1){
    		if(cum[i] - cum[res - 1] > best){
    			best = cum[i] - cum[res - 1];
    			taken = {res , i};
    ...
    ptr = ptr - (taken.yy - taken.xx + 1);


    for(int i = taken.xx , j = taken.yy + 1;i <= ptr;i++ ,j++){
        arr[i] = arr[j];
        cum[i] = cum[i - 1] + mp[arr[i]];
       // cout << arr[i] <<" " << cum[i] << endl; 
    ...
    for(int i = 1;i <= ptr;i++){
       ...
       // cout <<res << endl;
    	if(res != -1){
    		if(cum[res] - cum[i - 1] > best2){
    			best2 = cum[res] - cum[i - 1];
    	...
    	if(res != -1){
    		if(cum[i] - cum[res - 1] > best2){
    			best2 = cum[i] - cum[res - 1];
