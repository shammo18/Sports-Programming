...
void solve(){
   ...
   for(int mask = 1;mask < (1<<n);mask++){
   	 for(int i = 0;i < n;i++){
   	 	for(int j = 0;j < n;j++){
   	 		if(checkbit(mask , j)){
   	 			dp[i][mask] += dp[j][togglebit(mask , j)];
   ...
   for(int mask = 1;mask < (1<<n);mask++){
   	 if(bitno(mask) >= 3){
   	 	for(int i = 0;i < n;i++){
   	 		if(checkbit(mask , i) && vis[mask] == false){
   	 			cout <<togglebit(mask , i) <<" " << i <<" "  << dp[i][togglebit(mask , i)] << endl;
   	 			ans += dp[i][togglebit(mask , i)];
   	 			vis[mask] = true;
   	 			break;
   	 		}
   	 	}
