...
ll solve(ll x, ll y, ll m){
       ...
       if(x < 50 || y < 50 || x > 900 ||  y > 900)return 1e9;
       if(x == X && y == Y){
       	return dp[X][Y][m] = m;
       }
	   ...
	   ll ret = 1e9;
	   ret = min(ret,solve(x - POW(2LL,m),y,m + 1));
	   ret = min(ret,solve(x + POW(2LL,m),y,m + 1));
	   ret = min(ret,solve(x,y - POW(2LL,m),m + 1));
	   ret = min(ret,solve(x,y + POW(2LL,m),m + 1));
...
void solprint(ll x,ll y,ll m ,ll ans){
        //if(x < 0 || y < 0 || x >500 ||  y > 500)return;
	//cout << dp[x + POW(2LL,m)][y][m + 1]  << endl;
    if(x >  POW(2LL,m) &&  dp[x - POW(2LL,m)][y][m + 1] == ans){
    	    cout << 'W';
        	solprint(x - POW(2LL,m),y,m +  1,ans );
    }else 
        if(x  >  POW(2LL,m) && dp[x + POW(2LL,m)][y][m + 1] == ans){
            cout << 'E';
          	solprint(x + POW(2LL,m),y,m + 1,ans );  	
    }else 
        if(y >  POW(2LL,m) && dp[x][y - POW(2LL,m)][m + 1] == ans){
        	cout << 'S';
        	solprint(x,y - POW(2LL,m),m + 1,ans);    	
    }else 
        if(y >  POW(2LL,m) && dp[x][y + POW(2LL,m)][m + 1] == ans){
        	cout << 'N';
        	solprint(x,y + POW(2LL,m),m + 1 , ans);
...
int main(){
    ...
    while(t--){
    	...
    	X += 400;
    	Y += 400;
    	ll ans = solve(400,400,0);
    	if(ans <= 0 || ans == 1e9) {cout << "IMPOSSIBLE" <<endl; continue;}

    	//cout << ans <<endl;
       // cout << dp[200][200][0] << endl;

    	solprint(400,400,0,ans);
    	cout << endl;
    }
