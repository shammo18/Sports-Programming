...
ll solve(int r , int g , int b){
      ...
      if(r < s[0]){
         ret = max(ret , solve(r + 1, g , b));
         if(g < s[1]) ret = max(ret , solve(r + 1, g + 1, b) + adj[0][r] * adj[1][g]);
         if(b < s[2]) ret = max(ret , solve(r + 1, g, b + 1) + adj[0][r] * adj[2][b]);         
      }

     if(g < s[1]){
         ret = max(ret  , solve(r, g + 1, b));
         // if(g < s[1]) solve(r + 1, g + 1, b) + adj[0][r] * adj[1][g];
         if(b < s[2])ret = max(ret ,  solve(r, g + 1, b + 1) + adj[1][g] * adj[2][b]);         
      }

     if(b < s[2]){
         ret = max(ret , solve(r, g , b + 1));
         // if(g < s[1]) solve(r + 1, g + 1, b) + adj[0][r] * adj[1][g];
         // if(b < s[2]) solve(r + 1, g, b + 1) + adj[0][r] * adj[2][b];         
      }
