...
pii func(int j){
     ...
     vector<int> v(n + 5) , cum(n + 5);
     
     int first_one = -1;
     for(int i = 1;i <= n;i++){
     	v[i] = arr[i][j];
     	cum[i] = cum[i - 1] + v[i];
        if(first_one == -1 && v[i] == 1){
           first_one = i;
        }

     }
     
     if(first_one == -1) return {0 , 0};

     int id = -1;
     int mx = 0;
     int kk = min(k , n - first_one + 1);
     for(int i = first_one;i + kk - 1<= n;i++){
         int cnt = cum[i + kk - 1] - cum[i - 1];
         if(cnt > mx){
           mx = cnt;
           id = i;
         }
