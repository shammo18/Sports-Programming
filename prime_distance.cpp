...
void solve(){
    ...
    for(int i = 1;i < prime.size();i++){
    	...
    	if(mark[2 + prime[i]] == false){
    		ll k = m - 3;
    		if(k < 0) break;
    		//cout << prime[i] << endl;

    		ll temp = (n - 2 - prime[i]) * nCr(3LL + k - 1 , k) * 2LL;
    		temp %= mod;
    		ans += temp;
    ...
    for(int i = 1;i  < prime.size();i++){
       if(4 + prime[i] >= n) break;
    	if(mark[4 + prime[i]] == false && mark[2 + prime[i]] == false){
    		ll k = m - 4;
    		if(k < 0) break;
    		//cout << prime[i] << endl;
    		ans += (n - 4 - prime[i]) * nCr(4LL + k - 1 , k);
    		ans %= mod;
