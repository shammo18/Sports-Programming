...
void solve() {
  ...
  for (int mask = 0; mask < (1 << n); mask++) {
    int bitno = __builtin_popcount(mask);
    if (bitno == 1) {
      for (int j = 0; j < n; j++) {
        if (checkbit(mask , j) == 1) {
          dp[1][mask] = health[j];
        }
      }
    }
      ...
      if (bitno == count - 1) {
        for (int j = 0; j < n; j++) {
          if (checkbit(mask , j) == 0) {
            ...
            int cost = health[j] / mx;
            if (health[j] % mx != 0) cost++;
            cnt++;
            dp[count][togglebit(mask , j)] = min(dp[count][togglebit(mask , j)] , dp[count - 1][mask] + cost);
          }
        }
      }
    }
