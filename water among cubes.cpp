...
inline  bool margin(int x , int y){
	if(x == 0 || y == 0 || x == n - 1 || y == m - 1) return true;
	return false;
}

int fill_it(int res , int x , int y){
       ...
       while(!q.empty()){
       	  ...
       	  for(int i = 0;i < 4;i++){
       	  	 int X = dx[i] + u.xx;
       	  	 int Y = dy[i] + u.yy;
       	  	 if(valid(X, Y) && !global_vis[X][Y] && res - grid[X][Y] > 0){
       	  	 	global_vis[X][Y] = true;
...
bool can(int h , int x , int y){
       ...
       while(!q.empty()){
       	  ...
       	  for(int i = 0;i < 4;i++){
       	  	 ...
       	  	 if(valid(X, Y) && !vis[X][Y]  && h - grid[X][Y] > 0){
       	  	 	vis[X][Y] = true;
                  //cout << X << " " << Y <<" " << h - grid[X][Y] << endl;
       	  	 	if(margin(X , Y)){
       	  	 		return false;
       	  	 	}
...
void op(int x , int y){
	  ...
	  int lo = grid[x][y] , hi = 10000 , res = lo;
