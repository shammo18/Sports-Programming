...
*/
    ...
    vector<ll> temp;
    vector<ll> cum(2 * n) , precal(2 * n);

    for(ll i = 0;i < 2 * n;i++){
    	temp.pb(arr[i % n]);
    	if(i == 0) cum[i] = temp[i] , precal[i] = f(temp[i]);
    	else       cum[i] = cum[i - 1] + temp[i] , precal[i] = precal[i - 1] + f(temp[i]);
    }
    ...
    for(int i = 0;i < n;i++){
       ll shift = 0;
       int j = i;
       if(i > 0) shift = cum[i - 1];
       int pos = upper_bound(all(cum) , x + shift) - cum.begin();
       pos--;

       if(pos < 0){ans = max(ans , f(x)); continue;}

       ll curr = precal[pos] - (i - 1 < 0 ? 0 : precal[i - 1] );
       curr += f(x + shift - cum[pos]);
      // cout << curr << endl;
       ans = max(ans , curr);
    }
    ...
    for(int i = 0;i < n;i++){
       ...
       int pos = upper_bound(all(cum) , x + shift) - cum.begin();
       pos--;
       if(pos < 0){ans = max(ans , f(temp[i]) - f(temp[i] - x)); continue;}


       ll curr = precal[pos] - (i - 1 < 0 ? 0 : precal[i - 1] );
       curr += -f(temp[pos + 1] - (x + shift - cum[pos])) + f(temp[pos + 1]);
       // cout << curr << endl;
       ans = max(ans , curr);
