...
*/
    ...
    for(int i = 1;i < n * m;i++){
    	cum[i] = cum[i - 1] + arr[i];
    	if((arr[i] - arr[i - 1]) % d != 0) flag = false;
    }

    if(flag == false) return cout << -1 << endl ,0;
    
    int curr = 0;
    int ans = 1e9;
    for(int i = arr[0];i <= arr[n * m - 1];i += d){
       int curr = lower_bound(all(arr) , i) - arr.begin();

       int dan = 0;
       if(curr != 0)dan = cum[n * m - 1] - cum[curr - 1] - ((n * m - curr) * i);
       else dan = cum[n * m - 1] - ((n * m - curr) * i);

       int bam = 0;

       if(curr - 1 >= 0) bam = ((curr) * i) - cum[curr - 1];
       else bam = curr * i + 0;



      ans = min(ans , (dan + bam) / d);
