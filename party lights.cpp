...
cum[i][j][k] +=   cum[i - 1][j][k] + cum[i][j - 1][k] + cum[i][j][k - 1] - cum[i - 1][j - 1][k] - cum[i - 1][j][k - 1] - cum[i][j - 1][k - 1] + cum[i - 1][j - 1][k - 1];

           }
         }
       }

    	int lo = 0 , hi = 127 ,res = -1;
    ...
    while(lo <= hi){
    	...
    	for(int i = 1;i <= 128 - d;i++){
           for(int j = 1;j <= 128 - d;j++){
           	 for(int k = 1;k <= 128 - d;k++){
           	 	int x1 = i , x2 = i + d , y1 = j , y2 = j + d , z1 = k , z2 = k + d;
             
ll cnt = cum[x2][y2][z2] - cum[x1 - 1][y2][z2] - cum[x2][y1 - 1][z2] - cum[x2][y2][z1 - 1] + cum[x1 - 1][y1 - 1][z2] + cum[x1 - 1][y2][z1 - 1] + cum[x2][y1 - 1][z1 - 1] - cum[x1 - 1][y1 - 1][z1 - 1];
             //cout << y1 <<" " << y2 << endl;
             //cout << cnt << endl;
             if(cnt >= K){
             	flag = true;
