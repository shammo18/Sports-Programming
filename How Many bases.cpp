...
ll factorize(int n , int m  ,int T ) {
    ...
    for ( int i = 0; i < prime.size() && prime[i] <= sqrtn; i++ ) {
        if ( n % prime[i] == 0 ) {
            ...
            ret1 *= (ll)power * (ll)m / (ll)T  + 1LL;
            ret1 %= mod;

            ret2 *= (ll)power * (ll)m / (T + 1LL)  + 1LL;
            ret2 %= mod;
    ...
    if ( n != 1 ) {
            ret1 *= (ll)1 * (ll)m / (ll)T  + 1LL;
            ret1 %= mod;

            ret2 *= (ll)1 * (ll)m / (T + 1LL)  + 1LL;
            ret2 %= mod;

    }
    
    ll ans = (ret1 - ret2) % mod + mod;
    ans %= mod;
    return ans;
}
