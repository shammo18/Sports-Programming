...
*/
    ...
    sort(w.rbegin(),w.rend(),cmp2);
     
    double alpha = -real(w[0]);
    double beta  = -imag(w[0]);
    
    for(int i = 0;i < n;i++){
        point temp(real(w[i]) + alpha , imag(w[i]) + beta);
        v.pb(temp);
        point temp2(real(u[i]) + alpha , imag(u[i]) + beta);
        u[i] = temp2;
    ...
    for(int i = 0;i < n;i++){
       if(v[0] == u[i]){cout << i + 1 << " ";break;}
    }

    for(int i = 0;i < n;i++){
       if(v[n / 2] == u[i]){cout << i + 1 << endl;break;}
    }
