...
void init(){
    ...
    for(int i = 0;i < LG;i++){
        for(int j = 0;j < n + 1;j++) P[j][i] = -1 , rmq[j][i].xx = 2e9;
    }
    ...
    for(int  i=1; i < LG; i++){
        for(int j = 1;j < n + 1;j++)
            if (P[j][i-1] != -1){
                P[j][i] = P[P[j][i-1]][i-1];
                rmq[j][i] = {min(rmq[j][i-1].xx , rmq[P[j][i-1]][i-1].xx) , max(rmq[j][i-1].yy , rmq[P[j][i-1]][i-1].yy)};
            }
	}
...
pii lca(int u, int v){
    ...
    for(i=lg; i>=0; i--){
        if (lvl[u] - (1<<i) >= lvl[v]){
            mx = max(mx , rmq[u][i].yy);
            mn = min(mn , rmq[u][i].xx);
            u = P[u][i];
        }
    }
   ...
   // cout << lvl[u] << endl;
    for(i=lg; i>=0; i--){
        if (P[u][i] != -1 and P[u][i] != P[v][i]){
            mx = max(mx , rmq[u][i].yy) ; mx = max(mx , rmq[v][i].yy);
            mn = min(mn , rmq[u][i].xx) ; mn = min(mn , rmq[v][i].xx);
            u = P[u][i], v = P[v][i];
        } 
    }
   mx = max(mx , rmq[u][0].yy) ; mx = max(mx , rmq[v][0].yy); 
   mn = min(mn , rmq[u][0].xx) ; mn = min(mn , rmq[v][0].xx);
    return {mn ,mx};
}
