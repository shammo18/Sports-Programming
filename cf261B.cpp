...
vector<int> coin ,arr(55);
ll target;
ll n , p;
double fac[55];
ll dp[51][51][52];

ll go(int pos ,int cnt , int sum , int ex){
   
   if(pos == coin.size()){
     if(sum > target && cnt == ex){
     	//cout << sum << endl;
     	return 1;
     }
     else return 0;
   ...
   ll ret = 0;
   ret += go(pos + 1, cnt , sum , ex);
   if(cnt < ex && sum + coin[pos] <= p) ret += go(pos + 1, cnt + 1, sum + coin[pos] , ex);
   return dp[pos][cnt][sum] = ret;
}
 
void solve(){
   fac[0] = 1;
   for(ll i = 1;i <= 50;i++) fac[i] = fac[i - 1] * i;
   cin >> n;
   ll tot = 0;
   ...
   for(ll ex = 1;ex < n;ex++){
   	  for(ll wh = 0;wh < n;wh++){
          ...
          coin.clear();
          for(ll i = 0;i < n;i++){
             if(i != wh) coin.pb(arr[i]);
          }
         // debug() , dbg(coin);
          mem(dp , -1);
          ll cnt = go(0 , 0 , 0 , ex);
          //debug() , dbg(cnt);
          ans += ex * cnt * fac[ex] * fac[n - ex - 1];
   	  }
   }  
   coin = arr;
   target = p;
   ...
   ans += n * cnt * fac[n];
   cout << fixed << ans << endl;
   double avg = (double)ans / fac[n];
   cout << fixed << showpoint << setprecision(8) <<  avg << endl;
