...
void solve(){
    ...
    int n , k;
    cin >> n >> k;
    vector<int> arr(n + 2) , ans(n + 3);
    for(int i = 1;i <= n;i++) cin >> arr[i] , ans[i] = arr[i];
    k = min(k , n);
    ...
    for(int i = 0;i <= 30;i++){
    	...
    	vector<int> dan(n + 5  ,inf);
        int mn = inf;
    
    	for(int j = n;j >= 1;j--){
           if(checkbit(arr[j] , i) == false) mn = j;
           if(mn == inf) dan[j] = inf;
           else dan[j] = mn; 
           
    	}

    	vector<int> bam(n + 5 , inf);
    	...
    	for(int j = 1;j <= n;j++){
           if(dan[j] != inf) dp[j] = min(dp[j] , dan[j] - j);
           else if(dan[1] != inf) dp[j] = min(dp[j] , dan[1] + n - j);

           if(bam[j] != inf) dp[j] = min(dp[j] , j - bam[j]);
           else if(bam[n] != inf) dp[j] = min(dp[j] , n - bam[n] + j); 
           if(dp[j] <= k) if(checkbit(ans[j] , i) == true) ans[j] = togglebit(ans[j] , i);
    	}
