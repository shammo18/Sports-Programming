...
int query(int node , int b , int e , int k){
    ...
    if(seg[left] >= k){
      ...
      return  query(right , mid + 1, e , k - seg[left]);
    }
...
int main(){
    ...
    for(int i = 1;i < mxn;i += 2) arr.pb(i);
    for(int i = 1;i < arr.size();i++) update(1 , 1 , sz , i , 1); 
    ...
    while(1){
       ...
       if(cur > cnt) break;	
       int pos = query(1 , 1, sz , cur++);
       // cout << arr[pos] << endl;

       for(int i = arr[pos];i < mxn;i += arr[pos]){
       	  if(i > cnt) break;
       	  int pos2 = query(1 , 1 , sz , i--);
       	  update(1 , 1, sz , pos2 , 0);
       	  cnt--;
       }
    }
