...
void update(int node,int b,int e,int i, int j){
     ...
     seg[node].xx = (seg[left].xx - min(seg[left].xx,seg[right].yy)) + seg[right].xx;
     seg[node].yy = (seg[right].yy - min(seg[left].xx,seg[right].yy)) + seg[left].yy;
     
     bal[node] =  bal[left] + bal[right] + min(seg[left].xx,seg[right].yy);
}
...
pii query(int node,int b,int e,int i, int j){
     ...
     pii x = query(left,b,mid,i,j);
     pii y = query(right,mid + 1,e,i,j);
     
     res += 2 * min(x.xx , y.yy);
     pii ret;
     ret.xx = (x.xx - min(x.xx,y.yy)) + y.xx;
     ret.yy = (y.yy - min(x.xx,y.yy)) + x.yy;
...
int main(){
    ...
    for(int i = 0;i < n;i++) update(1 , 0 , n - 1, i , i);
    int q;
    cin >> q;
    while(q--){
    	int l , r;
