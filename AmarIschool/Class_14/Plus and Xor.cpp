...
ll setbit(ll mask,ll bit){ return mask | (1LL<<bit) ; }
ll clearbit(ll mask,ll bit){return mask & ~(1LL<<bit);}
ll togglebit(ll mask,ll bit){return mask ^ (1LL<<bit);}
...
int main(){
    ...
    // (x + y) = (x^y) + 2*(x&y)
    if(b > a || (a - b) & 1LL) cout<< -1 << endl;
    else cout << (a - b) / 2LL <<" " << (a + b) / 2LL << endl;
