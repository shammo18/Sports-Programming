...
vector<int> adj[mxn];int cost[mxn];int lvl[mxn];int par[mxn];ll ans = 0;
bool vis[mxn];
int val[mxn];
bool leaf[mxn];
...
void bfs(int s){
     ...
     while(!q.empty()){
     	...
     	for(auto v : adj[u]){
     		...
     		lvl[v] = lvl[u] + 1;
            if(lvl[v] & 1){
            	val[u] = min(cost[v] - cost[par[u]], val[u]);
            	cost[u] = val[u] + cost[par[u]];
            }else{
            	val[u] = cost[u] - cost[par[u]];
            }
            if(adj[v].size() == 1){
            		leaf[v] = true;
            }
...
*/
   ...
   int n;
    ...
    for(int i = 1;i <= n;i++){
    	...
    	if(leaf[i]){
    		if(lvl[i] & 1){
               val[i] = cost[i] - cost[par[i]];
    		}else val[i] = 0;
    	}

    	if(val[i] == 2e9 + 5 || val[i] < 0) return cout << -1 << endl , 0;
