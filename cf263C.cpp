...
void ok(int x , int y) {
	...
	for (int i = 0; i < g[x].size(); i++) {
		if (g[x][i] == y) flag = true;
	}

	if (flag == false) return;
	vector<int> v = {1 , x , y} , fr(mxn);
	vector<bool> taken(mxn);
	taken[1] = taken[x] = taken[y] = true;
...
int main() {
	...
	for (int i = 0; i < 2 * n; i++) {
		...
		g[u].pb(v);
		g[v].pb(u);
		if ((int)g[u].size() > 4 || (int)g[v].size() > 4) {
