...
void lol(int a, int b, int c){
    ...
    for(int i = 0;i < s.size();i++){
       ...
       auto posb = lower_bound(all(adj[b]) , i);
       if(posb == adj[b].end()) continue;
       
       auto posc = lower_bound(all(adj[c]) , *posb);
       if(posc == adj[c].end()) continue;

       ans = min(*posc - i + 1 , ans);
