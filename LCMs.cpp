...
int main(){
    ...
    ll n;
    cin >> n;
    for(ll i = 0;i < n;i++) cin >> arr , fr[arr]++;
    ...
    for(ll i = mxn - 1;i >= 1;i--){
    	exact[i] = SQR(sum[i]) % mod;

    	for(ll j = i + i;j < mxn;j += i){
    		if(exact[i] < exact[j]) exact[i] += mod;
    		exact[i] -= exact[j];
            
    	}
    	ans += (exact[i] * ModInv(i , mod)) % mod;
    	ans %= mod;
    	if(i*fr[i] > ans) ans += mod;
        ans = (ans - i*fr[i])% mod;
        
    }
    ans = (ans * ModInv(2LL , mod)) % mod;
