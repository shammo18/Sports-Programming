...
vector<int> adj[mxn];
int d[mxn] , f[mxn]  , cost[mxn],  pos[mxn] , lvl[mxn];
int tim;
vector<int> arr;
...
void update(int node,int b,int e,int i, int j, int x , int flag){
     ...
     update(left,b,mid,i,j,x , flag);
     update(right,mid + 1,e,i,j,x, flag);
...
ll query(int node,int b,int e,int i, int j , int flag){

   if(b > j || e < i) return 0;
   if(b >= i && e <= j){
   	     tempOdd  += seg[node].lazyOdd;
         tempEven += seg[node].lazyEven;
   ...
   ll x = query(left,b,mid,i,j,flag);
   ll y = query(right,mid + 1,e,i,j,flag);
...
*/
   ...
   while(q--){
       ...
       if(type == 1){
         ...
         update(1 , 0 , n - 1, pos[x] - ((f[x] - d[x] + 1) / 2) + 1 , pos[x] , val , type);
       }else{
       	 tempOdd = 0;
       	 tempEven = 0;
         sci(x);
         ...
         int ans = query(1 , 0 , n - 1, pos[x] , pos[x] ,type );
         ans += cost[x];
