...
*/
    ...
    while(t--){
        ...
        ll res = 0;
        ans[1] = cum[n];
        res = add(res, mul(ans[1] , 1LL , mod) , mod);



        for(ll len = 2;len <= n;len++){
           ans[len] = add(ans[len - 1] , cum[n] - cum[len - 1] , mod);
           ans[len] = sub(ans[len] , cum[n] - cum[n - len + 2 - 1] , mod);

           res = add(res, mul(ans[len] , len , mod) , mod);
