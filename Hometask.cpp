...
int go(int pos , int m){
    if(pos == arr.size()) {
    	if(m == 0) return dp[pos][m] = 0;
    	else return dp[pos][m] = -1e8;
    }
    
    if(dp[pos][m] != -1) return dp[pos][m];

	int ret = -2e9;
	ret = max(ret , go(pos + 1 , (m + arr[pos]) % 3) + 1);
	ret = max(ret , go(pos + 1, m));

	return dp[pos][m] = ret;
}
 
void sol_print(int pos , int m , int mx){
     
     if(pos == arr.size()) return;

	 if(dp[pos + 1][(m + arr[pos]) % 3] + 1 == mx){
	 	cout << arr[pos];
        sol_print(pos + 1 , (m + arr[pos]) % 3 , mx - 1);
	 }else{
        sol_print(pos + 1 , m , mx);
...
int main(){
    ...
    if(zero == n) return cout << 0 << endl , 0;
