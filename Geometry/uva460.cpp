...
void solve(){
   ...
   // if(one == 0 && two == 0 && three == 0 && four == 0) exit(0);
   rec a{one , two , three , four};
   cin >> one >> two >> three >> four;
   rec b{one , two , three , four};

   rec ans;

   ans.xl = max(a.xl , b.xl);
   ans.xh = min(a.xh , b.xh);
   ans.yl = max(a.yl , b.yl);
   ans.yh = min(a.yh , b.yh);

   if(ans.xh <= ans.xl || ans.yh <= ans.yl){
   	 cout << "No Overlap" << endl;
   }else{
   
   	 printf("%d %d %d %d\n" , ans.xl , ans.yl , ans.xh , ans.yh);
   }
