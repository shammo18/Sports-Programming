...
int solve(int pos , int flag){
	 ...
	 if(flag){
	   int cost = 0;
	   if(s[pos] == '0')cost++;
       ret = min(ret , solve(pos + 1,1) + cost);
       ret = min(ret, solve(-1 , 1) + cost + cum[n] - cum[pos]);
	 }else{
...
*/
    ...
    s = '0' + s;
    int l = -1  ,r = -1;
    for(int i = 0;i < s.size();i++) if(s[i] == '1'){l = i;break;}
    for(int i = n;i > 0;i--) if(s[i] == '1'){r = i;break;}
    for(int i = 1;i <= n;i++) cum[i] = cum[i - 1] + (s[i] - '0');
    if(l == -1 || cum[r] - cum[l - 1] == r - l + 1) return cout << 0 << endl , 0;


    cout << min(cum[n] , solve(1 , 0)) << endl;
