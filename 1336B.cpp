...
inline ll op(ll  x, ll y , ll z) {return SQR(x - y) + SQR(x - z) + SQR(z - y);}

ll machine(int u , int v , int w){
        ll x , y , z;
        ll ret = INF;
	    for(int i = 0;i < gems[v].size();i++){
	    	auto itr = lower_bound(all(gems[w]) , gems[v][i]);
	    	if(itr == gems[w].end()) continue;
	        z = *itr;
	        itr = upper_bound(all(gems[u]) , gems[v][i]);
	        if(itr == gems[u].begin()) continue;
	        itr--;
	        x = *itr;
	        y = gems[v][i];
            ret = min(ret , op(x , y , z));
	    }
	    // cout << ret << endl;
...
void solve(){
	   ...
	   for(int i = 0;i < 3;i++) sort(all(gems[i]));
       ...
       do{
          ...
          ans = min(ans , machine(v[0] , v[1] , v[2]));
