...
ll go(int pos , int prev , int is_start , int mask){
	if(pos == m - 1){
		if(prev == 10) return 0;
	}
   if(pos == m){
   ...
   if(dp[pos][prev][is_start][mask] != -1) return dp[pos][prev][is_start][mask];
   ll ret = 0;
   for(int i = 0;i < n;i++){
      	if(prev == 10 || abs(prev - i) == 1){
      		//ans.pb(i);
      		if(is_start == 0){
      			if(i == 0) ret = add(ret , go(pos + 1, 10 , 0 , mask));
      			else ret = add(ret , go(pos + 1, i , 1 , setbit(mask , i)));
      		}else ret = add(ret , go(pos + 1, i , 1, setbit(mask , i)));
      		//ans.pb(i);
      	    //ret += go(pos + 1, i);
      	   // ans.pop_back();
   ...
   return dp[pos][prev][is_start][mask] = ret;
}
