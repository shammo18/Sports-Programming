...
void solve(){
   ...
   for(int i = 2;i < n;i++){
      ...
      else if(prev != brr[i]) prev = brr[i];
      else if(prev != crr[i]) prev = crr[i];
      cout << prev << " ";
   }

   int i = n;

      if(prev != arr[i] && arr[i] != arr[1]) prev = arr[i];
      else if(prev != brr[i] && brr[i] != arr[1]) prev = brr[i];
      else if(prev != crr[i] && crr[i] != arr[1]) prev = crr[i];
      cout << prev << " ";  
