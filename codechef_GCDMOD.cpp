...
template <typename T> T BigMod (T b,T p,T m){if (p == 0) return 1;if (p%2 == 0){T s = BigMod(b,p/2,m);return (((__int128)s%m)*((__int128)s%m))%(__int128)m;}return (((__int128)b%m)*((__int128)BigMod(b,p-1,m)%m))%m;}
...
*/
    ...
    while(t--){
        ...
        ll x = ((__int128)BigMod(a , n , m) % m + (__int128)BigMod(b , n , m) % m) % m;
