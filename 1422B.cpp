...
void solve(){
   ...
   for(int i = 0;i < n;i++){
   	 for(int j = 0;j < m;j++){
   	 	...
   	 	if(vis[i][j]) continue;
   	 	vector<ll> v;
   	 	if(!vis[i][j]) v.pb(arr[i][j]) , vis[i][j] = true;
   	 	if(!vis[n - 1 - i][j]) v.pb(arr[n - 1 - i][j]) ,vis[n - 1 - i][j] = true;
   	 	if(!vis[i][m - 1 - j]) v.pb(arr[i][m - 1 - j]) , vis[i][m - 1 - j] = true;
   	 	if(!vis[n - 1 - i][m - 1 - j]) v.pb(arr[n - 1 - i][m - 1 - j]) , vis[n - 1 - i][m - 1 - j] = true;
