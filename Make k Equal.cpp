...
*/
    ...
    for(auto u : mp){
    	...
    	if(u.yy >= k) return cout << 0 << endl , 0;
    }
    ...
    for(ll i = 1;i <= n;i++){
    	...
    	if(i >= k) mn = min(mn , i * arr[i] - cum[i] - (i - k));
    	if((n - i + 1) >= k) mn = min(mn , cum2[i] - (n - i + 1) * arr[i] - (n - i + 1 - k));
    	mn = min(mn , i * arr[i] - cum[i] - (i - k) + cum2[i] - (n - i + 1) * arr[i] - (n - i + 1 - k) - k + 1);
