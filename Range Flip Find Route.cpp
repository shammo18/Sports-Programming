...
int bfs(){
      ...
      if(grid[0][0] == '.') lvl[0][0] = 0;
      else lvl[0][0] = 1;

      q.push({-lvl[0][0] , {0 , 0}});

	  while(!q.empty()){
	  	pair<int , pii> temp = q.top();
	  	q.pop();
	  	pii u = {temp.yy.xx , temp.yy.yy};
	  	...
	  	for(int i = 0;i < 2;i++){
	  		...
	  		if(valid(x , y)){
	  			int cost = 0;
	  			if(grid[u.xx][u.yy] == '.' && grid[x][y] == '#') cost = 1;
                if(lvl[u.xx][u.yy] + cost < lvl[x][y]){
                	lvl[x][y] = lvl[u.xx][u.yy] + cost;
                	q.push({-lvl[x][y] , {x , y}});
                }
	  		}
	  	}
