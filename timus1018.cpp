...
void dfs(int u , int par){
	...
	for(auto v : adj[u]){
		...
		dfs(v.xx , u);
		sub[u] += sub[v.xx];
		cost[u] += v.yy + cost[v.xx];
		if(left.xx == -1) left = v;
		else right = v;
	}


	if(left.xx == -1 && right.xx == -1) return;

	if(left.xx != -1 && right.xx == -1){
		for(int i = 0;i <= sub[left.xx];i++){
			dp[u][i]  = min(dp[u][i] , dp[left.xx][i]);
		}
	}


    if(left.xx != -1 && right.xx != -1){
	for(int i = 0;i <= sub[right.xx] - 1;i++)
	   dp[u][sub[left.xx] + i] = min(dp[u][sub[left.xx] + i] , dp[right.xx][i] + cost[left.xx] + left.yy);

    for(int i = 0;i <= sub[left.xx] - 1;i++)
	   dp[u][sub[right.xx] + i] = min(dp[u][sub[right.xx] + i] , dp[left.xx][i] +  cost[right.xx] + right.yy);
    

    for(int i = 0;i <= sub[left.xx] - 1;i++){
    	for(int j = 0;j <= sub[right.xx] - 1;j++){
    		dp[u][i + j] = min(dp[u][i + j] , dp[left.xx][i] + dp[right.xx][j]);
    	}
    }
    ...
    dp[u][sub[u] - 1] = min(dp[u][sub[u] - 1] , cost[u]);
