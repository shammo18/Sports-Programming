...
multiset<int> ms[mxn];
int col[mxn];   // 0 = black , 1 = white

 
void predfs(int u, int p, int h)
{
    P[u][0] = p;
    depth[u] = h;
       ...
       if(t == 0){
          if(col[v] == 0){
           ...
           int mn = 1e9;
            int x = v;
            while(x != -1){
            	if(ms[x].size() != 0) mn = min(mn , dis(v , x) + *(ms[x].begin()));
            	x = cenpar[x];
            }
