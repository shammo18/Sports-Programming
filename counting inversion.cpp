...
ll go(int pos , int isSmall , int isLarge , unordered_map<int , int> mp){
   
   if(pos == 14) return 0;

   int lo = x[pos] - '0' , hi = y[pos] - '0';
   if(isSmall) hi = 9;
   if(isLarge) lo = 0;
   ...
   ll ret = 0;
    ...
    {   ll cost = 0;
        ...
        ret += go(pos+1, (i<(y[pos]-'0'))?1:isSmall, (i>(x[pos]-'0'))?1:isLarge , mp) + cost;
        ans += ret;
        if(i == 0){
        	if(isLarge) mp[i]--;
        }
