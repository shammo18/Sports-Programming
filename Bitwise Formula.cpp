...
int func(int ck, int bit){
  ...
  for(int i = 0;i < ptr;i++){
     ...
     if(val[i][0] == '!'){
     	...
     	if(one[i] == -1) x = ck;
     	else if(val[one[i]][0] == '!') x = temp[bit][one[i]];
     	else x = val[one[i]][bit] - '0';

     	if(two[i] == -1) y = ck;
     	else if(val[two[i]][0] == '!') y = temp[bit][two[i]];
     	else y = val[two[i]][bit] - '0';

     	 // cout << x <<" sdfsd " << two[i] << endl;
...
int main(){
  ...
  for(int i = 0;i < n;i++){
  	...
  	if(var[0] == '0' || var[0] == '1'){
       ...
       cin >> var;
       if(var == "AND") op[ptr] = 0;
       else if(var == "OR") op[ptr] = 1;
       else if(var == "XOR") op[ptr] = 2;
