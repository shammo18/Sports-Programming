...
vector<pii> mpl[mxn], mpr[mxn];
int arr[mxn] , brr[mxn] , crr[mxn];
...
*/
  ...
  for(int i = 0;i < have.size();i++){
  	  ...
  	  for(int j = (int)mpl[have[i]].size() - 2;j >= 0;j--) mpl[have[i]][j].yy = min(mpl[have[i]][j + 1].yy , mpl[have[i]][j].yy);
  	  for(int j = 1;j < mpr[have[i]].size();j++)mpr[have[i]][j].yy = min(mpr[have[i]][j - 1].yy , mpr[have[i]][j].yy);
  ...
  int ans = INF;
  for(int i = 0;i < n;i++){
      int rem = x - (brr[i] - arr[i] + 1);
      if(rem < 0) continue;

      
      

      int lo = 0 , hi = (int)mpr[rem].size() - 1 , res = -1;

      while(hi >= 0 && lo <= hi){
      	int mid = (lo + hi) / 2;

      	if(mpr[rem][mid].xx < arr[i]){
      		res = mid;
      		lo = mid + 1;
      	}else hi = mid - 1;
      }
      ...
      if(res != -1){
      	ans = min(ans , crr[i] + mpr[rem][res].yy);
      }

      lo = 0 ; hi = (int)mpl[rem].size() - 1 ; res = -1;

      while(hi >= 0 && lo <= hi){
      	int mid = (lo + hi) / 2;

      	if(mpl[rem][mid].xx > brr[i]){
      		res = mid;
      		hi = mid - 1;
      	}else lo = mid + 1;
      }
      ...
      if(res != -1){
      	ans = min(ans , crr[i] + mpl[rem][res].yy);
      }
