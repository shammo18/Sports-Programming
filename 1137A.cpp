...
ordered_set <int> row[1002] , col[1002];
...
*/
     ...
     for(int i = 0;i < n;i++){
         for(int j = 0;j < m;j++){
             ...
             int r_pos = row[i].order_of_key(arr[i][j]);
             int c_pos = col[j].order_of_key(arr[i][j]);
             int r_sz = row[i].size();
             int c_sz = col[j].size();

             arr[i][j] = max(r_pos , c_pos) +  max(r_sz - r_pos , c_sz - c_pos);
