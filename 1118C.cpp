...
*/
   ...
   for(int row = 1;row <= n/2;row++){
     for(int col = 1;col <= n / 2;col++){
        ...
        // cout << v.size() << endl;
        int val = (*itr).yy;
        if(cnt < 4) return cout << "NO" << endl , 0;
        grid[row][col] = val;
        grid[row][n - col + 1] = val;
        grid[n - row + 1][col] = val;
        grid[n - row + 1][n - col + 1] = val;
        v.erase(itr);
        v.insert({(cnt - 4) * -1 , val});
     }
 }
     ...
     if(n & 1){
     	for(int col = 1;col <= n/2;col++){
            ...
            if(cnt < 2) return cout << "NO" << endl , 0;
     		grid[n/2 + 1][col] = grid[n / 2 + 1][n - col + 1] = val;
     		        v.erase(itr);
                    v.insert({(cnt - 2) * -1 , val});
     	}
        ...
        for(int row = 1;row <= n/2;row++){
            ...
            int val = (*itr).yy;
            if(cnt < 2) return cout << "NO" << endl , 0;
     		grid[row][n/2 + 1] = grid[n - row + 1][n / 2 + 1] = val;
     		   v.erase(itr);
               v.insert({(cnt - 2) * -1 , val});
     	}
