...
void bfs(int s , int id){
     ...
     for(int i = 0;i < mxn;i++) dis[id][i] = inf;
     dis[id][s] = 0;
     vis[s] = true;
     queue<int> q;
     q.push(s);
	...
	//scanf("%d %d %d %d", &m, &n, &k1, &k2);
      ...
      for(int i = 0;i < n;i++){
        for(int j = 0;j < m;j++){
           for(int k = -k1;k <= k1;k++){
             for(int l = -k1;l <= k1;l++){
                 if(valid(i + k , j + l) && abs(k) + abs(l) <= k1 && grid[i + k][j + l] == 0)
                    adj[i * m + j].pb((i + k) * m + j + l);
             }
           }
        }
      }
      ...
      for(int i = 0;i < n;i++){
        for(int j = 0;j < m;j++){
           for(int k = -k2;k <= k2;k++){
             for(int l = -k2;l <= k2;l++){
                 if(valid(i + k , j + l) && abs(k) + abs(l) <= k2 && grid[i + k][j + l] == 0)
                    adj[i * m + j].pb((i + k) * m + j + l);
             }
           }
        }
      ...
      int ans = inf;
        ...
        for(int j = 0;j < n * m;j++){
           if(grid[j / m][j % m] == 0)
           ans = min(ans , max(dis[0][j] ,dis[1][j]));
        }
