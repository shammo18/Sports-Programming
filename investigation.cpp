...
ll go(int pos , int is_small , int rem , int sum_rem){
    
    if(pos == num.size()){
    	if(rem == 0 && sum_rem == 0) return 1;
    	else return 0;
    }
    if(dp[pos][is_small][rem][sum_rem] != -1) return dp[pos][is_small][rem][sum_rem];
    ll ret = 0;
	int mx = is_small ? 9 : num[pos];
	for(int i = 0;i < mx;i++){
		ret += go(pos + 1, 1 , (rem * 10 + i) % k , (sum_rem + i) % k);
	}
	if(is_small) ret += go(pos + 1, 1 , (rem * 10 + mx) % k , (sum_rem + mx) % k);
	else ret += go(pos + 1, 0 , (rem * 10 + mx) % k , (sum_rem + mx) % k);
	return dp[pos][is_small][rem][sum_rem] = ret;
}
