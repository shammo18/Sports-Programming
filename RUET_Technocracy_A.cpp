...
void sieve(){
    ...
    prime.push_back(2);
    sp[2] = 2;
    for(int i = 3;i<N;i+=2)if(!mark[i])prime.push_back(i) , sp[i] = i;
}
...
void solve(){
    ...
    for(int r = 2;r <= m;r++){
       ...
       cur.pb({prev , cnt});
        ...
        int a1 , a2 , m1 = 1 , m2 = 1;
         x = v[cur[0].xx][cur[0].yy - 1];

        for(int i = 1;i < cur.size();i++){
            int p  , q;

            a2 = v[cur[i].xx][cur[i].yy - 1];
            a1 = x;
            m1 = m1 * po[cur[i - 1].xx][cur[i - 1].yy - 1] ;
            m2 = po[cur[i].xx][cur[i].yy - 1];

            int p = ext_gcd(m1 , m2 , p , q);
                       // cout << p << " " << q << endl;
            x = a1 * m2 * q + a2 * m1 * p;
            int cur_mod = m1 * m2;
            //cout <<"hi " << cur_mod << endl;
            while(x < 0) x += cur_mod;
            while(x >= cur_mod) x -= cur_mod;
